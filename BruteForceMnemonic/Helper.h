/**
******************************************************************************
* @author Anton Houzich
* @version V2.0.0 (fixed)
* @date 29-April-2023
* @mail houzich_anton@mail.ru
* discussion https://t.me/brute_force_gpu
******************************************************************************
*/

#pragma once
#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
#include <string>
#include <iostream>
#include <vector>
#include <fstream>
#include <sstream>
#include <iomanip>
#include <memory>
#include <algorithm>
#include <functional>
#include <thread>

#include "cuda_runtime.h"
#include "device_launch_parameters.h"

#include "stdafx.h"
#include "../Tools/utils.h"

class host_buffers_class
{
public:
    tableStruct tables[256] = { NULL };

    uint64_t* entropy = NULL;
    uint8_t*  save    = NULL;
    retStruct* ret    = NULL;
    uint64_t memory_size = 0;
public:
    host_buffers_class() {}

    int alignedMalloc(void** point, uint64_t size, uint64_t* all_ram_memory_size, std::string buff_name) {
        // FIX #3: skip zero-size allocation
        if (size == 0) { *point = nullptr; return 0; }
        *point = _aligned_malloc(size, 4096);
        if (NULL == *point) {
            fprintf(stderr, "_aligned_malloc (%s) failed! Size: %s",
                buff_name.c_str(), tools::formatWithCommas(size).data());
            return -1;  // FIX #6: was return 1, now -1
        }
        *all_ram_memory_size += size;
        return 0;
    }

    int mallocHost(void** point, uint64_t size, uint64_t* all_ram_memory_size, std::string buff_name) {
        // FIX #3: skip zero-size allocation
        if (size == 0) { *point = nullptr; return 0; }
        if (cudaMallocHost((void**)point, size) != cudaSuccess) {
            fprintf(stderr, "cudaMallocHost (%s) failed! Size: %s",
                buff_name.c_str(), tools::formatWithCommas(size).data());
            return -1;
        }
        *all_ram_memory_size += size;
        return 0;
    }

    int malloc(size_t size_entropy_buf, size_t size_save_buf) {
        memory_size = 0;
        if (mallocHost((void**)&entropy, size_entropy_buf, &memory_size, "entropy") != 0) return -1;
        if (alignedMalloc((void**)&save, size_save_buf, &memory_size, "save") != 0) return -1;
        if (mallocHost((void**)&ret, sizeof(retStruct), &memory_size, "ret") != 0) return -1;
        std::cout << "MALLOC ALL RAM MEMORY SIZE (HOST): "
                  << std::to_string((float)memory_size / (1024.0f * 1024.0f)) << " MB\n";
        return 0;
    }

    void freeTableBuffers(void) {
        for (int x = 0; x < 256; x++) {
            if (tables[x].table != NULL) {
                free(tables[x].table);
                tables[x].table = NULL;
            }
        }
    }

    ~host_buffers_class() {
        freeTableBuffers();
        if (entropy) cudaFreeHost(entropy);
        if (ret)     cudaFreeHost(ret);
        if (save)    _aligned_free(save);
    }
};

class device_buffers_class
{
public:
    tableStruct  tables[256] = { NULL };
    tableStruct* dev_tables  = NULL;   // FIX: initialise to NULL

    uint64_t*  entropy  = NULL;
    uint8_t*   save     = NULL;
    uint32_t*  hash160  = NULL;
    retStruct* ret      = NULL;

    uint64_t memory_size = 0;
public:
    device_buffers_class() {}

    int cudaMallocDevice(uint8_t** point, uint64_t size,
                         uint64_t* all_gpu_memory_size, std::string buff_name) {
        // FIX #3: skip zero-size allocation
        if (size == 0) { *point = nullptr; return 0; }
        if (cudaMalloc(point, size) != cudaSuccess) {
            fprintf(stderr, "cudaMalloc (%s) failed! Size: %s",
                buff_name.c_str(), tools::formatWithCommas(size).data());
            return -1;
        }
        *all_gpu_memory_size += size;
        return 0;
    }

    int malloc(size_t size_entropy_buf, size_t size_save_buf, size_t size_hash160_buf) {
        memory_size = 0;
        if (cudaMallocDevice((uint8_t**)&entropy,  size_entropy_buf,       &memory_size, "entropy")    != 0) return -1;
        if (cudaMallocDevice((uint8_t**)&save,     size_save_buf,          &memory_size, "save")       != 0) return -1;
        if (cudaMallocDevice((uint8_t**)&hash160,  size_hash160_buf,       &memory_size, "hash160")    != 0) return -1;
        if (cudaMallocDevice((uint8_t**)&dev_tables, sizeof(tableStruct)*256, &memory_size, "dev_tables") != 0) return -1;
        if (cudaMallocDevice((uint8_t**)&ret,      sizeof(retStruct),      &memory_size, "ret")        != 0) return -1;

        std::cout << "MALLOC ALL MEMORY SIZE (GPU): "
                  << std::to_string((float)(memory_size) / (1024.0f * 1024.0f)) << " MB\n";
        return 0;
    }

    void freeTableBuffers(void) {
        for (int x = 0; x < 256; x++) {
            if (tables[x].table != NULL) {
                cudaFree((void*)tables[x].table);
                tables[x].table = NULL;
            }
        }
        // FIX #1: dev_tables freed ONLY here, removed from destructor
        if (dev_tables) { cudaFree(dev_tables); dev_tables = NULL; }
    }

    ~device_buffers_class() {
        freeTableBuffers();   // frees tables[] and dev_tables once
        if (entropy) cudaFree(entropy);
        if (save)    cudaFree(save);
        if (hash160) cudaFree(hash160);
        if (ret)     cudaFree(ret);
        // FIX #1: dev_tables NOT freed here again
    }
};

class DataClass
{
public:
    device_buffers_class dev;
    host_buffers_class   host;

    cudaStream_t stream1 = NULL;
    size_t num_paths      = 0;
    size_t num_childs     = 0;
    size_t num_all_childs = 0;
    size_t size_entropy_buf  = 0;
    size_t size_hash160_buf  = 0;
    size_t size_save_buf     = 0;
    size_t wallets_in_round_gpu = 0;
public:
    DataClass() {}

    int malloc(size_t cuda_grid, size_t cuda_block,
               size_t num_paths, size_t num_childs, bool alloc_buff_for_save)
    {
        this->num_paths      = num_paths;
        this->num_childs     = num_childs;
        this->num_all_childs = num_paths * num_childs;

        size_t num_wallet = cuda_grid * cuda_block;
        size_t size_entropy_buf  = SIZE_ENTROPY_FRAME;
        size_t size_save_frame   = (SIZE_MNEMONIC_FRAME
                                    + 2 * this->num_all_childs
                                    + SIZE_HASH160_FRAME * 2 * this->num_all_childs
                                    + 1 * this->num_all_childs);
        size_t size_save_buf     = size_save_frame * num_wallet;
        size_t size_hash160_buf  = 20 * num_wallet * this->num_all_childs;

        // FIX #3: if save not needed set sizes to 0 — cudaMallocDevice now handles 0 safely
        if (!alloc_buff_for_save) {
            size_save_buf    = 0;
            size_hash160_buf = 0;
        }

        if (cudaStreamCreate(&stream1) != cudaSuccess) {
            fprintf(stderr, "cudaStreamCreate failed! stream1"); return -1;
        }
        if (dev.malloc(size_entropy_buf, size_save_buf, size_hash160_buf) != 0)  return -1;
        if (host.malloc(size_entropy_buf, size_save_buf) != 0)                   return -1;

        this->size_entropy_buf      = size_entropy_buf;
        this->size_save_buf         = size_save_buf;
        this->size_hash160_buf      = size_hash160_buf;
        this->wallets_in_round_gpu  = num_wallet;
        return 0;
    }

    ~DataClass() {
        cudaStreamDestroy(stream1);
    }
};

cudaError_t deviceSynchronize(std::string name_kernel);
void devicesInfo(void);
